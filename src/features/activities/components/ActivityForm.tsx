...
const schema = z.object({ type: z.enum(activityTypes), personId: z.string(), occurredAt: z.string(), notes: z.string(), outcome: z.string(), nextActionDate: z.string() });
...
function initial(value?: Partial<Activity>): FormData {
  return { type: value?.type ?? 'CALL', personId: value?.personId ?? '', occurredAt: toLocalDateTime(value?.occurredAt), notes: value?.notes ?? '', outcome: value?.outcome ?? '', nextActionDate: toLocalDateTime(value?.nextActionDate) };
}
...
export default function ActivityForm({ mode, initialValues, people = [], isPeopleLoading = false, isSubmitting = false, errorMessage, onSubmit, onCancel }: ActivityFormProps) {
  ...
  const submit = (data: FormData) => onSubmit({ type: data.type, personId: nullable(data.personId), occurredAt: nullableDate(data.occurredAt), notes: nullable(data.notes), outcome: nullable(data.outcome), nextActionDate: nullableDate(data.nextActionDate) });
