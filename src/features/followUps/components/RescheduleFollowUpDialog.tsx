...
export default function RescheduleFollowUpDialog({ followUp, open, onClose }: { followUp: FollowUpActivity; open: boolean; onClose: () => void }) {
  ...
  return <Dialog open={open} onClose={close} fullWidth maxWidth="sm"><DialogTitle>زمان‌بندی مجدد</DialogTitle><DialogContent><Stack spacing={2} sx={{ pt: 1 }}>{mutation.isError && <Alert severity="error">{apiMessage || 'خطا در زمان‌بندی مجدد پیگیری.'}</Alert>}<TextField required label="تاریخ پیگیری بعدی" type="datetime-local" value={date} onChange={(e) => setDate(e.target.value)} error={Boolean(validation)} helperText={validation} slotProps={{ inputLabel: { shrink: true } }} /><TextField label="یادداشت زمان‌بندی مجدد" multiline minRows={3} value={note} onChange={(e) => setNote(e.target.value)} /></Stack></DialogContent><DialogActions><Button onClick={close} disabled={mutation.isPending}>انصراف</Button><Button variant="contained" onClick={submit} disabled={mutation.isPending}>{mutation.isPending ? 'در حال ثبت...' : 'زمان‌بندی مجدد'}</Button></DialogActions></Dialog>;
}
