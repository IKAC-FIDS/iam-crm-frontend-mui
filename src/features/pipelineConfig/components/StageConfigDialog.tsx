...
export default function StageConfigDialog({ stage, open, onClose }: { stage: PipelineStageConfig; open: boolean; onClose: () => void }) {
  ...
  const submit = async () => {
    try { await mutation.mutateAsync({ stage: stage.code, payload: { label: label.trim(), description: description.trim() || undefined, sortOrder: Number(sortOrder), color: color || undefined, isActive, isTerminal } }); toast.success('تنظیمات مرحله بروزرسانی شد.'); onClose(); }
