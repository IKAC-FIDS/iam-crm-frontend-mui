// ============================================================
// مسیر: src/features/pipelineConfig/components/StageConfigDialog.tsx
// ============================================================
...
export default function StageConfigDialog({ stage, open, onClose }: StageConfigDialogProps) {
  ...
  const submit = async () => {
    try {
      ...
      toast.success('تنظیمات مرحله بروزرسانی شد.');
      onClose();
    } catch {
  ...
  return (
    <Dialog open={open} onClose={() => !mutation.isPending && onClose()} fullWidth maxWidth="sm">
      <DialogTitle>ویرایش مرحله</DialogTitle>

      <DialogContent>
        <Stack spacing={2} sx={{ pt: 1 }}>
          {mutation.isError && (
            <Alert severity="error">ذخیره تنظیمات مرحله با خطا مواجه شد.</Alert>
          )}

          <TextField label="کد مرحله" value={stage.code} disabled />
          ...
          <TextField
            label="رنگ"
            placeholder="#001736"
            helperText="برای ذخیره رنگ مرحله، مقدار hex وارد کنید. نمونه: #001736"
            value={color}
          ...
          <FormControlLabel
            ...
            control={
              <Switch
                checked={isTerminal}
                onChange={(_, checked) => setIsTerminal(checked)}
              />
            }
          />
        </Stack>
