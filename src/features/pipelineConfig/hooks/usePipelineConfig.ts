...
export function useUpdatePipelineStage() { const client = useQueryClient(); return useMutation({ mutationFn: ({ id, payload }: { id: string; payload: UpdatePipelineStagePayload }) => pipelineConfigService.updateStage(id, payload), onSuccess: () => Promise.all([client.invalidateQueries({ queryKey: pipelineConfigQueryKeys.stages() }), client.invalidateQueries({ queryKey: ['pipeline'] })]) }); }
