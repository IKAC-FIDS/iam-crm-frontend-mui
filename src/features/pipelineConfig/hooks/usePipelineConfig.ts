...
export function useReorderPipelineStages() { const invalidate = useInvalidateAll(); return useMutation({ mutationFn: (p: ReorderStagesPayload) => pipelineConfigService.reorderStages(p), onSuccess: invalidate }); }
function useInvalidateRules() { const invalidate = useInvalidateAll(); return invalidate; }
export function useCreateTransitionRule() { const invalidate = useInvalidateRules(); return useMutation({ mutationFn: (p: TransitionRulePayload) => pipelineConfigService.createTransitionRule(p), onSuccess: invalidate }); }
