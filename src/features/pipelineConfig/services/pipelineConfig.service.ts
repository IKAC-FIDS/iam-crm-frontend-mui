...
const list = (payload: unknown): unknown[] => { const data = unwrapApiResponse<unknown>(payload); if (Array.isArray(data)) return data; return data && typeof data === 'object' && 'items' in data && Array.isArray((data as { items: unknown }).items) ? (data as { items: unknown[] }).items : []; };
...
export const pipelineConfigService = {
  ...
  getStage: async (id: string) =>
    stage(unwrapApiResponse<unknown>((await axiosInstance.get(`/admin/pipeline/stages/${id}`)).data)),

  createStage: async (payload: CreatePipelineStagePayload) =>
    stage(unwrapApiResponse<unknown>((await axiosInstance.post('/admin/pipeline/stages', payload)).data)),

  updateStage: async (id: string, payload: UpdatePipelineStagePayload) =>
    stage(unwrapApiResponse<unknown>((await axiosInstance.patch(`/admin/pipeline/stages/${id}`, payload)).data)),
