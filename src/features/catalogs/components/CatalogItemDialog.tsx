...
export default function CatalogItemDialog({ kind, group, item, open, onClose }: { kind: CatalogKind; group?: LookupGroup; item: CatalogItem | null; open: boolean; onClose: () => void }) {
  ...
  const coded = kind === 'leadSources' || kind === 'lookupOptions'; const university = kind === 'universities'; const categorized = kind === 'painPoints' || kind === 'useCases'; const persona = kind === 'personas';
  const payload = (): CatalogPayload => {
    if (kind === 'industries') return { name: primary.trim(), description: description.trim() || undefined };
    if (kind === 'universities') return { name: primary.trim(), code: code.trim() || undefined, description: description.trim() || undefined, isActive };
