...
import { Alert, Box, Chip, FormControl, InputLabel, MenuItem, Paper, Select, TextField, Typography } from '@mui/material';
...
export default function PeopleDirectoryPage() {
  ...
  return <Box sx={{ minWidth: 0 }}><Typography variant="h4" sx={{ mb: 0.5 }}>افراد</Typography><Typography color="text.secondary" sx={{ mb: 2 }}>فهرست یکپارچه افراد شرکت‌ها بر اساس سطح دسترسی شما.</Typography>
    <Paper sx={{ p: 2, mb: 2 }}><Typography variant="h6" sx={{ mb: 2 }}>فیلتر افراد</Typography><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)', lg: 'repeat(3, 1fr)' }, gap: 2 }}>
      ...
      <CompanyAutocomplete value={company} onChange={(value) => { setCompany(value); resetPage(); }} label="شرکت" />
    ...
    </Box>{(reportOptions.isError || departmentsQuery.isError || jobTitlesQuery.isError || personaRolesQuery.isError || seniorityLevelsQuery.isError) && <Alert severity="warning" sx={{ mt: 2 }}>دریافت بخشی از گزینه‌های فیلتر با خطا مواجه شد.</Alert>}</Paper>
