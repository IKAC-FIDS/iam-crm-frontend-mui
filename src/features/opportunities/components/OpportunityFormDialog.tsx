...
export default function OpportunityFormDialog({ companyId, opportunity, open, onClose }: { companyId: string; opportunity?: Opportunity | null; open: boolean; onClose: () => void }) {
  ...
  return <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm"><DialogTitle>{opportunity ? 'ویرایش فرصت' : 'ایجاد فرصت'}</DialogTitle><DialogContent><OpportunityForm key={opportunity?.id ?? 'new'} companyId={companyId} opportunity={opportunity} stages={(stages.data ?? []).filter((s) => s.isActive)} onChange={setPayload} /></DialogContent><DialogActions><Button onClick={onClose}>انصراف</Button><Button variant="contained" disabled={pending || !payload.title?.trim()} onClick={submit}>ذخیره</Button></DialogActions></Dialog>;
}
