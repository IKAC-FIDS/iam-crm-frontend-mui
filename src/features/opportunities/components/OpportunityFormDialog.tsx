...
export default function OpportunityFormDialog({ companyId, opportunity, open, onClose }: { companyId: string; opportunity?: Opportunity | null; open: boolean; onClose: () => void }) {
  ...
  const submit = async () => { if (!payload.title.trim()) return; try { if (opportunity) { await update.mutateAsync({ id: opportunity.id, payload: { title: payload.title, description: payload.description, priority: payload.priority, estimatedValue: payload.estimatedValue, expectedCloseDate: payload.expectedCloseDate, source: payload.source, sourceOptionId: payload.sourceOptionId, opportunitySource: payload.opportunitySource, primaryContactId: payload.primaryContactId, probability: payload.probability, competitor: payload.competitor } }); } else await create.mutateAsync(payload); toast.success(opportunity ? 'فرصت ویرایش شد.' : 'فرصت ایجاد شد.'); onClose(); } catch (error) { toast.error(getApiErrorMessage(error, 'ذخیره فرصت انجام نشد.')); } };
  const pending = create.isPending || update.isPending;
  const mutationError = update.error ?? create.error;
  return <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm"><DialogTitle>{opportunity ? 'ویرایش فرصت' : 'ایجاد فرصت'}</DialogTitle><DialogContent>{mutationError && <Alert severity="error" sx={{ mt: 1 }}>{getApiErrorMessage(mutationError, 'ذخیره فرصت انجام نشد.')}</Alert>}<OpportunityForm key={opportunity?.id ?? 'new'} companyId={companyId} opportunity={opportunity} stages={(stages.data ?? []).filter((s) => s.isActive)} onChange={setPayload} /></DialogContent><DialogActions><Button onClick={onClose}>انصراف</Button><Button variant="contained" disabled={pending || !payload.title?.trim()} onClick={submit}>ذخیره</Button></DialogActions></Dialog>;
}
