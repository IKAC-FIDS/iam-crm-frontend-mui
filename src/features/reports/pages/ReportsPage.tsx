...
import type { PeriodComparisonFilters, ReportFilters } from '../types/report.types';
...
function comparisonFromUrl(params: URLSearchParams): Pick<PeriodComparisonFilters, 'comparisonMode' | 'compareStartDate' | 'compareEndDate'> { return { comparisonMode: (params.get('comparisonMode') as PeriodComparisonFilters['comparisonMode']) ?? 'PREVIOUS_PERIOD', compareStartDate: params.get('compareStartDate') ?? undefined, compareEndDate: params.get('compareEndDate') ?? undefined }; }
...
function setComparisonParams(params: URLSearchParams, filters: PeriodComparisonFilters) { const mode = filters.comparisonMode ?? 'PREVIOUS_PERIOD'; if (mode === 'PREVIOUS_PERIOD') params.delete('comparisonMode'); else params.set('comparisonMode', mode); if (mode === 'CUSTOM') { if (filters.compareStartDate) params.set('compareStartDate', filters.compareStartDate); if (filters.compareEndDate) params.set('compareEndDate', filters.compareEndDate); } else { params.delete('compareStartDate'); params.delete('compareEndDate'); } }
...
export default function ReportsPage() {
  ...
  const [comparisonDraft, setComparisonDraft] = useState(() => comparisonFromUrl(params)); const [comparisonFilters, setComparisonFilters] = useState(() => comparisonFromUrl(params));
  ...
  const quality = useDataQualityReport(filters, hasAccess && tab === 'data-quality'); const comparison = usePeriodComparisonReport({ ...filters, ...comparisonFilters }, hasAccess && tab === 'comparison');
  ...
  const apply = () => { const next = { ...draft }; setFilters(next); if (tab === 'comparison') setComparisonFilters(comparisonDraft); updateParams((p) => { setFilterParams(p, next); if (tab === 'comparison') setComparisonParams(p, { ...next, ...comparisonDraft }); p.set('agingPage', '1'); }); };
  const reset = () => { const next = defaults(), comparisonNext = { comparisonMode: 'PREVIOUS_PERIOD' as const }; setDraft(next); setFilters(next); setComparisonDraft(comparisonNext); setComparisonFilters(comparisonNext); setSelectedCompanies([]); setParams(new URLSearchParams(), { replace: true }); };
  return <Box sx={{ minWidth: 0 }}><Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ mb: 3, justifyContent: 'space-between', alignItems: { sm: 'center' } }}><Box><Typography variant="h4">گزارش‌ها</Typography><Typography color="text.secondary">گزارش‌های مدیریتی مبتنی بر داده‌های تجمیعی سمت سرور.</Typography></Box><Stack direction="row"><ReportExportButton tab={tab} filters={tab === 'comparison' ? { ...filters, ...comparisonFilters } : filters} /><Button variant="outlined" startIcon={<RefreshIcon />} onClick={() => queryClient.invalidateQueries({ queryKey: reportQueryKeys.all })}>به‌روزرسانی گزارش‌ها</Button></Stack></Stack>
    ...
    <Stack spacing={4}><ReportFilterPanel tab={tab} draft={tab === 'comparison' ? { ...draft, ...comparisonDraft } : draft} selectedCompanies={selectedCompanies} options={options.data} isLoading={options.isLoading} isError={options.isError} isApplying={allQueries.some((q) => q.isFetching)} onChange={(next) => { const { comparisonMode, compareStartDate, compareEndDate, ...common } = next; setDraft(common); setComparisonDraft({ comparisonMode: comparisonMode ?? 'PREVIOUS_PERIOD', compareStartDate, compareEndDate }); }} onCompaniesChange={(companies) => { setSelectedCompanies(companies); setDraft((f) => ({ ...f, companyIds: companies.map((c) => c.id) })); }} onApply={apply} onReset={reset} />
