...
export function MetricHelpButton({ label, help }: MetricHelpButtonProps) {
  ...
  return <>
    ...
    <Popover id={id} open={Boolean(anchor)} anchorEl={anchor} onClose={close} anchorOrigin={{ vertical: 'bottom', horizontal: 'left' }} transformOrigin={{ vertical: 'top', horizontal: 'left' }} slotProps={{ paper: { role: 'dialog', 'aria-label': help.title, sx: { direction: 'rtl', textAlign: 'right', width: 'min(400px, calc(100vw - 32px))', maxWidth: 400, p: 2, overflowWrap: 'anywhere', '& .MuiTypography-root': { textAlign: 'right' } } } }}><Typography variant="h6">{help.title}</Typography><Typography variant="body2" sx={{ mt: 1.25, lineHeight: 2, direction: 'rtl', textAlign: 'right', unicodeBidi: 'plaintext' }}>{help.description}</Typography></Popover>
  </>;
}
