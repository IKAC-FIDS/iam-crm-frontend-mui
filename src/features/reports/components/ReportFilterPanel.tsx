...
export default function ReportFilterPanel({ draft, selectedCompanies, options, isLoading, isError, isApplying, onChange, onCompaniesChange, onApply, onReset }: Props) {
  const chosen = (key: keyof ReportFilters, available: ReportFilterOption[]) => { const values = draft[key]; return available.filter((item) => Array.isArray(values) && values.includes(item.value)); };
