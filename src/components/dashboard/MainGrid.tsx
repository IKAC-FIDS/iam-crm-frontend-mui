...
export default function MainGrid() {
  ...
  const canOpportunity = can(user, 'opportunity:view'), canTask = can(user, 'task:view'), canMeeting = can(user, 'meeting:view'), canActivity = can(user, 'activity:view', ['ADMIN', 'MANAGER', 'REP']), canProductView = can(user, 'product:view');
  ...
  return <Stack spacing={3} sx={{ width: '100%', mt: 2 }}>
    ...
    {data && <><Typography color="text.secondary">آخرین تولید گزارش: {formatJalaliDateTime(data.generatedAt)}</Typography>
      ...
      ]} /> : <Alert severity="info">خلاصه مالی در اطلاعات فعلی موجود نیست.</Alert>}
      ...
      ]} /> : <Alert severity="info">خلاصه قیمت‌گذاری در اطلاعات فعلی موجود نیست.</Alert>}
      ...
      ]} /> : <Alert severity="info">خلاصه کانال‌های فروش در اطلاعات فعلی موجود نیست.</Alert>}
      ...
      ]} /><Button component={RouterLink} to="/reports?tab=data-quality">مشاهده گزارش کیفیت داده</Button></> : <Alert severity="info">خلاصه کیفیت داده‌های سازمان در اطلاعات فعلی موجود نیست.</Alert>}
